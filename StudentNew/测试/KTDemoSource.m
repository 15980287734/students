...
@implementation KTDemoSource

- (void)dealloc {
   [lists_ release], lists_ = nil;
   [super dealloc];
}
...
- (id)init {
   ...
   if (self) {
      ...
      lists_ = [[NSArray alloc] initWithObjects:
                 [NSArray arrayWithObjects:@"http://mongol.cri.cn/mmsource/images/2014/04/11/2014041103fengli.jpg", @"http://mongol.cri.cn/mmsource/images/2014/04/11/2014041103fengli.jpg", nil],
...
#pragma mark -
#pragma mark KTDemoSource

- (NSInteger)numberOfOnes {
   NSInteger count = [lists_ count];
   return count;
}
/////////////////////////////////////
//自定义界面和单纯image选择一个实现，imageAtIndex优先选择
- (id)detailAtIndex:(NSInteger)index {
   NSMutableArray *imageUrls = [lists_ objectAtIndex:index];
    KTDemoView *view=[[KTDemoView alloc]initWithFrame:CGRectMake(20, 70, 280, 300)];
   [view setIndexData:imageUrls];
    
    NSLog(@"detailAtIndex回调%ld",(long)index);
    return view;
}
//专门用于imageview
/*
...
{
    NSArray *imageUrls = [lists_ objectAtIndex:index];
    NSString *url = [imageUrls objectAtIndex:FULL_SIZE_INDEX];
    NSLog(@"imageAtIndex=%d=%@",index,url);
    UIImage *img=[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:url]]];
    return img;
}
 */
//////////////////////////////////
- (void)getAnswerAtIndex:(NSInteger)index withbutton:(ButtomBarButton *)btn
{
    NSLog(@"getAnswerAtIndex=%ld",(long)index);
}
- (void)restoreQuestionAtIndex:(NSInteger)index withbutton:(ButtomBarButton *)btn
{
    NSLog(@"restoreQuestionAtIndex=%ld",(long)index);
    [btn setTitle:@"333"];
}
-(void)jumpAtIndex:(NSInteger)index
        withbutton:(ButtomBarButton *)btn
{
    NSLog(@"jumpAtIndex=%ld",(long)index);
